...
### ✨ 体验与配置优化
- **ASR & LLM 核心配置项恢复与优化**: 恢复 ASR 和 LLM 工作模式的选择下拉菜单，支持用户在在线 API 和本地模型之间灵活切换；为本地 ASR 配置项添加了原生跨平台目录选择（“📁 选择目录”）按钮，彻底支持 Windows、macOS 和 Linux。
...
- **卡片折行与排版优化**: 将 LibraryView 和 WorkstationView（网格与列表）中的节目系列名称与单集标题拆分为上下两行显示，节目名称作为红色小号粗体标签（严格截断），单集标题分别使用 truncate/line-clamp-2 保证整体排版一致。
